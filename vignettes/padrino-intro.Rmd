...
`RPadrino` provides an interface to the PADRINO database. PADRINO houses metadata on published Integral Projection Models, and all the information needed to rebuild them. `RPadrino` provides a set of functions that wrap around [`ipmr`](https://levisc8.github.io/ipmr/) so that you can rebuild these models using _R_. Additionally, there's some data downloading and management functionality, as well as tools to help report and cite studies used in an analysis.
...
# As above, remove the [1:4] to see all plots and use par() to control their
# arrangement
# par(mfrow = c(3, 2))
...
We saw above that sometimes data in PADRINO can give values that make no sense. PADRINO is committed to providing IPMs *as they are published*, and does not take a stance on the technical correctness of these models. `aaa341` does provide an opportunity to illustrate how to address some of these issues. 

We saw that that the fundamental operator contained negative values. This shouldn't be possible, but occurs when the determinant of $(I - P)$ is negative. This usually occurs when some part of the probability of survival function is exactly 1 (i.e. individuals of that size cannot die). We can correct this by modifying the functional form in PADRINO to compute the parallel minimum of the survival function the authors reported and some value that is really close to 1, but not quite there. 
