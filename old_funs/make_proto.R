...
#' @importFrom rlang quo .data enquos !!! is_empty
#' @export
...
make_proto_ipm <- function(db, ...) {
  ...
  for(i in seq_len(length(unique(db[[1]]$ipm_id)))) {
    ...
    if(.has_ranefs(use_db)) {
      use_db <- .split_ranef_exprs(use_db)
    }
    ...
    for(j in seq_along(parsed_K$kernel_flags)) {
      ...
      proto_ipm_params <- RPadrino:::.associate_vr_parameters(use_db,
                                                              parsed_kernel) #%>%
      # RPadrino:::.add_environmental_parameters(use_db)
