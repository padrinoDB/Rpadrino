#' @title adds environmental parameters (if present) to a kernel parameter tree.
#'
#' @param param_list A list of kernel parameters. Always should be the output
#' of \code{.associate_vr_parameters}
#' @param db The database object to search for the environmental parameters in.
#'
#' @return A list with the same structure as \code{param_list}, but with entries
#' corresponding to parameters and expressions in the \code{db}'s
#' \code{EnvironmentalVariable} table.
#'

.add_environmnetal_parameters <- function(param_list, db) {

  env_table <- db[[length(db)]]
...
.kernel_vr_LHS_RHS_mat <- function(db, parsed_K, name) {
  ...
  kernel_funs <- expr_table$formula[RPadrino:::.identify_kernels(name,
...
.identify_kernels <- function(flag, kernels) {
  x <- strsplit(kernels,';')
  x <- apply(x, MARGIN = 2, FUN = function(y) trimws(y))
  if(!is.matrix(x)) {
